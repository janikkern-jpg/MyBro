...
// (Cloudflare Pages Functions). GET auf /v1/models bei beiden Anbietern –
// validiert nur den API-Key und verbraucht keine Tokens.
...
async function checkOpenAI(key: string | undefined): Promise<ProviderStatus> {
  if (!key) return { connected: false, configured: false };
  try {
...
export const onRequestGet: PagesHandler = async ({ env }) => {
  const [anthropic, openai] = await Promise.all([
    ...
    checkOpenAI(env.OPENAI_API_KEY),
  ]);
